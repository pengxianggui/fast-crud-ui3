...
| existsUrl(`1.5.12+`)       | 存在性校验地址(列启用唯一性校验时会用到)。如需自定义则配置                             | `String`                                                  | `${baseUrl}/exists`                                                                                 |
| enableDblClickEdit         | 启用双击编辑                                                     | `Boolean`/`Function<() => Boolean>`                       | true                                                                                                |
| enableMulti                | 启用多选                                                       | `Boolean`/`Function<() => Boolean>`                       | true                                                                                                |
| enableIndex(`1.5.9+`)      | 启用序号列                                                      | `Boolean`/`Function<() => Boolean>`                       | false                                                                                               |
| enableColumnFilter         | 启用列动筛。若设置false则所有列不支持表头点击筛选                                | `Boolean`/`Function<() => Boolean>`                       | true                                                                                                |
| enableFilterCache(`1.5.13+`) | 启用筛选缓存(会话有效)                                               | `Boolean`/`Function<() => Boolean>`                       | true                                                                                                |
| listCacheSeconds(`1.5.35+`)  | `/list`接口响应的缓存时长(秒), 用于下拉选项等场景防止高频重复请求; 设为0可关闭缓存。缓存key会覆盖合并固定conds后的完整查询条件 | `Number`                                                 | 10                                                                                                  |
| lazyLoad                   | 分页数据延迟加载。若设置true则表格渲染后不会自动加载数据，得人为触发                       | `Boolean`/`Function<() => Boolean>`                       | false                                                                                               |
...
| exportable(`1.5.12+`)      | 是否可导出。若为false则关闭导出入口                                       | `Boolean`/`Function<() => Boolean>`                       | true                                                                                                |
| idField(`1.5.12+`)         | 主键字段,默认为id(列组件配置unique时必须指定)。                              | `String`                                                  | `id`                                                                                                |
| createTimeField            | 创建时间字段。若配置则激活内置存筛                                          | `String`                                                  | -                                                                                                   |
...
| showQuickFilterBtn(`1.5.25+`)            | 快筛表单是否单独显示搜索/重置按钮                                                                                                         | `Boolean`        | `false`     |
...
|extra(`1.5.32+`)|自定义扩展条件，详见上面的[condExtra](#condExtra)|`Array`|-|

:::warning
1. 在迭代过程中, `FastTable`可能会不断变化, 某个筛选属性可能以前有，现在没了，也记得调整此condGroups值，否则这个存筛项会被标记为不兼容，呈现禁用状态。
2. extra 不像 conds, 相同的属性可以重复, 对于同时生效的多个存筛，如果都有相同的 extra 属性定义，则后面的覆盖前面的。
:::
